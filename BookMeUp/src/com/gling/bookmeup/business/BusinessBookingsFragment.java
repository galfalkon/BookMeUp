...
public class BusinessBookingsFragment extends OnClickListenerFragment {
	...
	private GenericMultiChoiceCardArrayAdapter<Booking> _pendingBookingsAdapter,
			_approvedBookingsAdapter;

	private Button _btnPending, _btnApproved;
	private ViewSwitcher _viewSwitcher;

	private CardListViewWrapperView _pendingBookingsListView,
			_approvedBookingsListView;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		_pendingBookingsAdapter = new GenericMultiChoiceCardArrayAdapter<Booking>(
				getActivity(), _pendingBookings,
				new PendingBookingCardGenerator(),
				R.menu.business_booking_list_pending_mutlichoice) {
			@Override
		...
		_approvedBookingsAdapter = new GenericMultiChoiceCardArrayAdapter<Booking>(
				getActivity(), _approvedBookings,
				new ApprovedBookingCardGenerator(),
				R.menu.business_booking_list_approved_mutlichoice) {
			@Override
			public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
				switch (item.getItemId()) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		_btnPending = (Button) view
				.findViewById(R.id.business_bookings_btnPending);
		_btnApproved = (Button) view
				.findViewById(R.id.business_bookings_btnApproved);

		_pendingBookingsListView = (CardListViewWrapperView) view
				.findViewById(R.id.business_bookings_cardListViewPendingBookings);
		_pendingBookingsListView.setAdapter(_pendingBookingsAdapter);
		_pendingBookingsListView
				.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE_MODAL);

		_approvedBookingsListView = (CardListViewWrapperView) view
				.findViewById(R.id.business_bookings_cardListViewApprovedBookings);
		_approvedBookingsListView.setAdapter(_approvedBookingsAdapter);
		_approvedBookingsListView
				.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE_MODAL);

		_viewSwitcher = (ViewSwitcher) view
				.findViewById(R.id.business_bookings_viewSwitcher);
	...
	private void inflateListWithFutureBookings() {
		ParseQuery<Booking> query = new ParseQuery<Booking>(Booking.CLASS_NAME)
				.whereEqualTo(Booking.Keys.BUSINESS_POINTER,
						Business.getCurrentBusiness()).whereGreaterThan(
						Booking.Keys.DATE, new Date());
		...
		query.findInBackground(new FindCallback<Booking>() {
			...
			public void done(List<Booking> objects, ParseException e) {
				Log.i(TAG, "Done querying future bookings. #objects = "
						+ objects.size());
				if (e != null) {
	...
	private void updatePendingBookingsTitleAndDisplayMode() {
		// Update title
		_btnPending.setText(String.format("%s (%d)",
				getString(R.string.business_bookings_list_pending_header),
				_pendingBookings.size()));

		// Update display mode
		DisplayMode newDisplayMode = _pendingBookings.isEmpty() ? DisplayMode.NO_ITEMS_VIEW
	...
	private void updateApprovedBookingsTitleAndDisplayMode() {
		// Update title
		_btnApproved.setText(String.format("%s (%d)",
				getString(R.string.business_bookings_list_approved_header),
				_approvedBookings.size()));

		// Update display mode
		DisplayMode newDisplayMode = _approvedBookings.isEmpty() ? DisplayMode.NO_ITEMS_VIEW
				: DisplayMode.LIST_VIEW;
		_approvedBookingsListView.setDisplayMode(newDisplayMode);
	}

	private class PendingBookingCardGenerator implements
			ICardGenerator<Booking> {
		@Override
		public Card generateCard(final Booking booking) {
			...
			card.setTitle("Service: " + booking.getServiceName() + "\n"
					+ "Date: "
					+ Constants.DATE_FORMAT.format(booking.getDate()));

			card.setOnLongClickListener(new OnLongCardClickListener() {
				@Override
				public boolean onLongClick(Card card, View view) {
					return _pendingBookingsAdapter
	...
	private class ApprovedBookingCardGenerator implements
			ICardGenerator<Booking> {
		@Override
		public Card generateCard(final Booking booking) {
			CardHeader cardHeader = new CardHeader(getActivity());
			...
			card.setOnLongClickListener(new OnLongCardClickListener() {
				@Override
				public boolean onLongClick(Card card, View view) {
					return _approvedBookingsAdapter
							.startActionMode(getActivity());
				}
			});
	...
	private void handleApprovalOfSelectedBookings() {
		...
		List<Booking> selectedBookings = _pendingBookingsAdapter
				.getSelectedItems();
		for (Booking booking : selectedBookings) {
			...
			PushUtils.notifyCustomerAboutApprovedBooking(booking,
					new SendCallback() {
						@Override
						public void done(ParseException e) {
							Log.i(TAG,
									"notifyCustomerAboutApprovedBooking done");
							if (e != null) {
	...
	private void handleCancellationOfSelectedBookings() {
		...
		List<Booking> selectedBookings = _approvedBookingsAdapter
				.getSelectedItems();
		for (Booking booking : selectedBookings) {
			...
			PushUtils.notifyCustomerAboutCanceledBooking(booking,
					new SendCallback() {
						@Override
						public void done(ParseException e) {
							Log.i(TAG,
									"notifyCustomerAboutCanceledBooking done");
							if (e != null) {
								Log.e(TAG, "Exception: " + e.getMessage());
								return;
							}
