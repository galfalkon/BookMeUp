...
public class BusinessProfileWizardActivity extends FragmentActivity implements
	...
	private AbstractWizardModel _wizardModel = new BusinessProfileWizardModel(
			this);
	...
	private void saveProfile() {
		final ProgressDialog progressDialog = ProgressDialog.show(_context,
				null, getString(R.string.progress_dialog_saving_profile));

		new AsyncTask<Void, Void, Void>() {
			@Override
			protected Void doInBackground(Void... params) {
				Business business = Business.getCurrentBusiness();
				business.setName(_wizardModel
						.findByKey(BusinessProfileWizardModel.GENERAL_INFO)
						.getData().getString(NameDescriptionPage.NAME_DATA_KEY));

				String description = _wizardModel
						.findByKey(BusinessProfileWizardModel.GENERAL_INFO)
						.getData()
				...
				String categoryName = _wizardModel
						.findByKey(BusinessProfileWizardModel.CATEGORY)
				...
				business.setPhoneNumber(_wizardModel
						.findByKey(BusinessProfileWizardModel.DETAILS)
						.getData()
						.getString(PhoneOpeningHoursPage.PHONE_DATA_KEY));

				String openingHours = _wizardModel
						.findByKey(BusinessProfileWizardModel.DETAILS)
						.getData()
				...
				byte[] scaledImage = _wizardModel
						.findByKey(BusinessProfileWizardModel.IMAGE).getData()
		...
		_stepPagerStrip
				.setOnPageSelectedListener(new StepPagerStrip.OnPageSelectedListener() {
					...
					public void onPageStripSelected(int position) {
						position = Math.min(_pagerAdapter.getCount() - 1,
								position);
	...
	private void updateBottomBar() {
		...
		if (position == _currentPageSequence.size()) {
			...
			_nextButton.setTextAppearance(this,
